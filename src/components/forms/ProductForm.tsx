...
const ProductForm: React.FC<ProductFormProps> = ({ product, onSubmit, onCancel, isLoading = false }) => {
  ...
  const form = useForm<ProductFormData>({
    ...
    defaultValues: {
      ...
      type: product?.type || '',
      compatibilite: product?.kind === 'Accessoire' ? product.compatibilite : '',
      dioptrie: product?.kind === 'Lentille' ? product.dioptrie : 0,
      dureeVie: product?.kind === 'Lentille' ? product.dureeVie : '',
      couleur: (product?.kind === 'Lentille' || product?.kind === 'Lunette') ? product.couleur : '',
      forme: product?.kind === 'Lunette' ? product.forme : '',
      matiere: product?.kind === 'Lunette' ? product.matiere : '',
      genre: product?.kind === 'Lunette' ? product.genre : 'Mixte',
      model3d: product?.kind === 'Lunette' ? product.model3d : '',
    },
  });
